...
type AddBackupConnectionPageProps = {
    errors: ErrorBag,
    flash?: {
        message: string,
        created: boolean,
...
export default function AddBackupConnection({ }: Props) {

    const { flash, errors: pageError }: AddBackupConnectionPageProps = usePage().props
    ...
    const { data, setData, post, errors, setError } = useForm<UseFormDataProps>({
        slug: '',
        connection_name: '',
    ...
    async function pushRequest(prevalidate = false) {
        post(route('connection.store', { prevalidate }), {
            preserveScroll: true,
            preserveState: true,
            onBefore: () => {
                setProcessing(true)
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4 overflow-y-scroll pt-80">
                    <Dialog.Panel className='mx-auto max-w-xl bg-white rounded px-5'>
                        <div className='py-5'>
                            ...
                            <Dialog.Description>
                                <span>
                                    Creating new database connection and backup configuration
                                </span>
                            </Dialog.Description>
                        </div>
                        ...
                        <div className='form'>
                            ...
                            <div className='mt-3'>
                                ...
                                <label htmlFor="notifiable" className="relative inline-flex cursor-pointer items-center">
                                    <input
                                        value={data.notifiable.toString()}
                                        onChange={(e) => {
